...
`autopvs1-link` is a thin FastMCP backend in the GeneFoundry fleet. It is unauthenticated
...
```bash
gh api -X PATCH repos/berntpopp/autopvs1-link \
...
```bash
gh api repos/berntpopp/autopvs1-link --jq '.security_and_analysis'
```
