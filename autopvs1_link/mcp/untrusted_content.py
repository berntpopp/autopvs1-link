...
def sanitize_message(text: str) -> str:
    ...
    A defensive backstop applied to EVERY caller-visible error message/detail
    string. A hostile upstream (or a caller-influenced 4xx/5xx error body) must
    never smuggle control, zero-width, bidirectional, or NUL code points into an
    error frame that reaches the model. Caller-visible messages are
    server-authored guidance data (never instructions, never fenced objects);
    upstream response bodies are additionally kept out of them at the source
    (Surface A, the API client). Reuses the same ``FORBIDDEN_CODEPOINTS`` set the
    untrusted-text fence removes so the two paths stay in lockstep.
    """
