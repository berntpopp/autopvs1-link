...
_ERROR_HANDLING_GUIDANCE = (
    "Error handling (branch on the canonical error_code; error_subcode "
    "carries the specific reason):\n"
    "- error_code == 'invalid_input': ask the user to confirm inputs "
    "(error_subcode names the field, e.g. invalid_variant_id, "
    "invalid_cnv_id, invalid_genome_build).\n"
    ...
    "'requires_disambiguation'): pick one of details.candidates and "
    "re-call.\n"
    "- error_code in {upstream_unavailable, rate_limited}: retry once "
    "after meta.retry_after_ms ('rate_limited' means AutoPVS1 throttled "
    "you — back off longer).\n"
...
def _classify_cnv_body(genome_build: str, cnv_id: str) -> str:
    return (
        ...
        "coordinate format (or the call returns error_subcode "
        "invalid_cnv_id), "
