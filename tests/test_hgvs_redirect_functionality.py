...
from autopvs1_link.api.autopvs1_client import AutoPVS1Client, HGVS_PATTERNS
...
class TestHGVSPatternDetection:
    ...
    def test_hgvs_pattern_constants(self):
        ...
        assert len(HGVS_PATTERNS) >= 6
        assert any("NM_" in pattern for pattern in HGVS_PATTERNS)
        assert any("c\\." in pattern for pattern in HGVS_PATTERNS)
        assert any("p\\." in pattern for pattern in HGVS_PATTERNS)
    ...
    async def test_detect_hgvs_pattern_transcript_level(self):
        ...
        try:
            ...
            for non_hgvs in non_hgvs_cases:
                assert not client._detect_hgvs_pattern(non_hgvs), f"Incorrectly detected as HGVS: {non_hgvs}"
    ...
    async def test_extract_variant_from_redirect_url(self):
        ...
        try:
            test_cases = [
                ...
                (
                    "/variant/hg19/2-48033984-G-GGATT",
                    ("hg19", "2-48033984-G-GGATT")
                ),
                (
                ...
                (
                    "https://autopvs1.bgi.com/search?q=BRCA1",
                    (None, None)
                ),
                (
                    "invalid-url",
...
class TestRedirectDetection:
    ...
    async def test_search_with_redirect_detection_hgvs(self):
        ...
        try:
            with patch("autopvs1_link.utils.retry_handler.retry_handler.http_request_with_retry", return_value=mock_response):
                result = await client.search_with_redirect_detection("NM_000128.3:c.1716+1G>A", "hg19")
    ...
    async def test_resolve_hgvs_notation_failure(self):
        ...
        try:
            with patch("autopvs1_link.utils.retry_handler.retry_handler.http_request_with_retry", return_value=mock_response):
                with pytest.raises(ValueError, match="did not resolve to a single variant"):
...
class TestServiceIntegration:
    ...
    async def test_service_search_with_redirect_detection(self):
        ...
        from autopvs1_link.models.autopvs1_models import VariantInfo, PVS1Flowchart
        ...
        mock_client.search_with_redirect_detection.assert_called_once_with("NM_000128.3:c.1716+1G>A", "hg19")
...
class TestEdgeCases:
    ...
    async def test_borderline_hgvs_patterns(self):
        ...
        try:
            ...
            borderline_cases = [
                ("c.123", True),  # Valid minimal coding notation
                ("p.Met1", True),  # Valid minimal protein notation
                ("NM_123456.1:c.456", True),  # Valid transcript with position
                ("c", False),  # Too short
                ("p", False),  # Too short
                ("NM_", False),  # Incomplete
                ("c.123ABC", True),  # Actually matches c.\d+ pattern - this is expected behavior
            ]
